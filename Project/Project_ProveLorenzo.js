...
class GameLogic{
  ...
  constructor(){
    ...
    this.objects = this.createObjects(NUM_MINES*4, this.p);
  ...
  //CREAZIONE DEGLI OGGETTI MINA, CRISTALLO E MURO 

  //Creo Variabili
  createVariables(NUM_MINES){
    let x_rand = [];
    let y_rand = [];
    for (let i = 0; i < NUM_MINES*4; i++) { //es: 10 mine, 10 cristalli & 20 muri
      x_rand[i] = floor(random(20, width-20));
      y_rand[i] = floor(random(20, width-20));
    }
    return {x_rand, y_rand};
  }

  //Creo Oggetti
  createObjects(numObj, player, x_rand, y_rand) {  //passo un numero di oggetti totali che voglio e il player
    let mines = []; let crystals = []; let randomWalls = [];

    for (var i=0; i < numObj.slice(0, length(numObj)/4) - 1; i++ ){
       mine[j] = new Mine(x_rand[i], y_rand[i])}

    for (var i=length(numObj)/4; i < numObj.slice(length(numObj)/4, length(numObj)/2) - 1 ; i++ ){
        crystal[j] = new Crystal(x_rand[i], y_rand[i])}
       
    for (var i=length(numObj)/4; i < numObj.slice(length(numObj)/4, length(numObj)); i++ ){
        randomWalls[j] = new RANDOM_Wall(x_rand[i], y_rand[i])}
  
   return {mines, crystals, randomWalls};
  }
